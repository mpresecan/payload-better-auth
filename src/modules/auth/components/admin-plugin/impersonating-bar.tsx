...
import { useBetterAuth } from "../../lib/context"
...
export function ImpersonatingBar() {
  ...
  const impersonatedUser =
    session && "impersonatedBy" in session?.session ? session?.user : null
  const impersonatedBy =
    session && "impersonatedBy" in session?.session
      ? (session?.session.impersonatedBy as string)
      : null
